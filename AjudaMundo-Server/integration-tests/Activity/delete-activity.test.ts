...
describe('DELETE Delete Activity', () => {
  let activityData = {
    name: 'Teste',
    points: 20,
  ...
  test('should delete an activity', async () => {
    ...
    const response = await request(app).delete(`/activity/${createdActivity.body.result.activityId}`).set('Authorization', `Bearer ${ongAuthToken}`);

    expect(response.status).toBe(204)
  })

  test('ong should not delete an activity that is not theirs', async () => {
    const createdActivity = await request(app).post('/activity').send(activityData).set('Authorization', `Bearer ${ongAuthToken}`);

    const response = await request(app).delete(`/activity/${createdActivity.body.result.activityId}`).set('Authorization', `Bearer ${secondOngAuthToken}`);

    expect(response.status).toBe(403)
    expect(response.body.message).toBe('Somente a ong que criou a atividade pode deleta-la')
  })

  test('user should not delete an activity', async () => {
    const createdActivity = await request(app).post('/activity').send(activityData).set('Authorization', `Bearer ${ongAuthToken}`);

    const response = await request(app).delete(`/activity/${createdActivity.body.result.activityId}`).set('Authorization', `Bearer ${userAuthToken}`);

    expect(response.status).toBe(428)
  })

  test('should not delete a done activity', async () => {
    const createdActivity = await request(app).post('/activity').send(activityData).set('Authorization', `Bearer ${ongAuthToken}`);
    ...
    expect(response.body.message).toBe('Atividades finalizadas não podem ser deletadas')
  })

  test('should not delete an assigned activity', async () => {
    const createdActivity = await request(app).post('/activity').send(activityData).set('Authorization', `Bearer ${ongAuthToken}`);
    ...
    expect(response.status).toBe(409)
    expect(response.body.message).toBe('Atividades atribuídas não podem ser deletadas')
  })
